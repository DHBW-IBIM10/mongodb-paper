...
\begin{savenotes} %allows proper citation marks inside tables
\begin{table}[ht]
...
\bottomrule 
\end{tabular*}
  \caption{SQL terms mapped to MongoDB source:
...
Similarly it is possible to query for values which are stored in an array instead of a subdocument.
If the array contains sub-documents it is also possible to query for specific fields within the sub-documents.\footcite[Cf.][10]{mongo_crud_manual}
This is demonstrated in Lst. \ref{lst:array-query}
...
db.inventory.find( { type:'food', $or:[ { qty:{ $gt:100} },{ price:{ $lt:9.95} } ]} )
    \end{javascriptcode}
    \caption{Exemplary usage of query operators\footcite[][9]{mongo_crud_manual}}
...
\bottomrule 
\end{tabular*}
  \caption{Query selectors for value comparison (taken from \url{http://docs.mongodb.org/manual/reference/operator/#query-selectors})}
