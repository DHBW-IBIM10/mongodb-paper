...
configuration file.
When installed on Linux, MongoDB comes with a default configuration file at
...
\begin{savenotes}
\begin{table}[htbp]
\begin{tabular*}{\textwidth}{p{0.2\textwidth} p{0.85\textwidth}}
\toprule
	\textbf{Name} 					& \textbf{Description}\\
	\midrule 
...
The driver allows interacting with MongoDB through Ruby by employing similar constructs to the examples above using the Mongo shell, however most developers use an Object Document Mapper(ODM) to gain higher-level abstractions and modelling functionality.\footcite[Cf.][]{mongo_ruby_driver}
