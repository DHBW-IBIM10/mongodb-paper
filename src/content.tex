...
\ref{lst:mongo-root}. Note that the creation of an administration user
is only possible when working on localhost with Mongo's localhost authentication
...
username and password. Lst. \ref{lst:add-user} shows how a user can be added to
the enron database.
...
> db.addUser({user:"mike", pwd:"abc123", roles: ["foo", "bar"]})
{
	"user" : "mike",
...
1
    \end{javascriptcode}
    \caption{Adding a user to the enron database}
    \label{lst:add-user}
\end{listing}


\ref{lst:mongo-auth} shows how one can
